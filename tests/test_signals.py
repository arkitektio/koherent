...
def request_context(db):
    """A full request-like auth context: user, client, org, token and task."""
    ...
    payload = TaskPayload(id="task-s", args={}, user="1", app="testapp", action="hash")
    ...
    resets = [
        ...
        (current_task_payload, current_task_payload.set(payload)),
...
def no_context(db):
    ...
    resets = [
        ...
        (current_task_payload, current_task_payload.set(None)),
        (current_task, current_task.set(None)),
    ]
    yield
    for var, reset in resets:
